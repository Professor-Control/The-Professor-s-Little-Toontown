...
Splats = {'tart': (0.3, FruitpieColor),
 ...
 'waterBalloon': (1.0, VBase4(0.75, 0.75, 1.0, 0.8))}
Variants = ('tart',
 'fruitpie',
 'splat-tart',
 'dust',
...
class PropPool:
    ...
    def __init__(self):
        ...
        self.propTypes[propName] = 'model'
        propName = 'blackball'
        self.propStrings[propName] = (self.getPath(5, 'baseball'),)
        self.propTypes[propName] = 'model'
        propName = 'restraining-order'
        self.propStrings[propName] = (self.getPath(3.5, 'shredder-paper-mod'),)
        self.propTypes[propName] = 'model'
        propName = 'blacklist'
        self.propStrings[propName] = (self.getPath(3.5, 'shredder-paper-mod'),)
        self.propTypes[propName] = 'model'
        propName = 'cold-caller-phone'
        self.propStrings[propName] = (self.getPath(3.5, 'phone'),)
        self.propTypes[propName] = 'model'
        propName = 'cold-caller-receiver'
        self.propStrings[propName] = (self.getPath(3.5, 'receiver'),)
        self.propTypes[propName] = 'model'
    ...
    def makeVariant(self, name):
        if name == 'tart':
            ...
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == 'restraining-order':
            tex = loader.loadTexture('phase_5/maps/restraining_order.png')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == 'blacklist':
            tex = loader.loadTexture('phase_5/maps/blacklist.png')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == 'cold-caller-phone':
            tex = loader.loadTexture('phase_5/maps/cold_caller_phone.png')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == 'cold-caller-receiver':
            tex = loader.loadTexture('phase_5/maps/cold_caller_phone.png')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
