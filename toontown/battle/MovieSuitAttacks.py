...
def doSuitAttack(attack):
    ...
    if name == ACID_RAIN:
        ...
        suitTrack = doBlackball(attack)
    elif name == BLACKLIST:
        suitTrack = doBlacklist(attack)
    elif name == BLOOD_MONEY:
        suitTrack = doBloodMoney(attack)
    elif name == BLUEPRINT:
        suitTrack = doDefault(attack)
    elif name == BOUNCE_CHECK:
        suitTrack = doBounceCheck(attack)
    elif name == BRAIN_FREEZE:
        suitTrack = doBrainFreeze(attack)
    ...
    elif name == COLD_HARD_CASH:
        suitTrack = doColdHardCash(attack)
    elif name == CONTRACT:
        suitTrack = doContract(attack)
    elif name == COUNTERCLAIM:
        suitTrack = doCounterclaim(attack)
    elif name == CRUNCH:
    ...
    elif name == DOWN_PAYMENT:
        suitTrack = doDownPayment(attack)
    elif name == EVICTION_NOTICE:
        suitTrack = doEvictionNotice(attack)
    elif name == EVIDENCE:
        suitTrack = doEvidence(attack)
    elif name == EVIL_EYE:
        suitTrack = doEvilEye(attack)
    elif name == EVIL_EYE_WSI:
        suitTrack = doEvilEyeWSI(attack)
        ...
        suitTrack = doGuiltTripWSI(attack)
        ...
        suitTrack = doJump(attack)
    elif name == LEGALESE:
        suitTrack = doLegalese(attack)
    elif name == LEGAL_TENDER:
        suitTrack = doDefault(attack)
    elif name == LIQUIDATE:
        suitTrack = doLiquidate(attack)
    elif name == MARKET_CRASH:
        suitTrack = doMarketCrash(attack)
    elif name == MARKETING_SPIN:
        suitTrack = doMarketingSpin(attack)
    elif name == MIND_BLOW:
        suitTrack = doDefault(attack)
    elif name == MUD_SLING:
        suitTrack = doMudSling(attack)
    elif name == MULLIGAN:
        ...
        suitTrack = doPeckingOrderWSI(attack)
    ...
    elif name == POSITIVE_REINFORCEMENT:
        suitTrack = doPositiveReinforcement(attack)
    elif name == POUND_KEY:
        ...
        suitTrack = doPowerTripWSI(attack)
    elif name == QUAKE:
        suitTrack = doQuake(attack)
    elif name == RAIN_OF_FIRE:
        suitTrack = doDefault(attack)
        ...
        suitTrack = doRestrainingOrderWSI(attack)
    elif name == ROLODEX:
...
def doDefault(attack):
    ...
    if suitName == 'f':
        ...
        attack['id'] = FILL_WITH_LEAD
        attack['name'] = 'FillWithLead'
        attack['animName'] = 'pencil-sharpener'
        return doFillWithLead(attack)
    elif suitName == 'ym':
        ...
        attack['id'] = SHRED
        attack['name'] = 'Shred'
        attack['animName'] = 'shredder'
        return doShred(attack)
    elif suitName == 'mm':
        attack['id'] = DEMOTION
        ...
        attack['id'] = THROW_GEAR
        attack['name'] = 'ThrowGear'
        attack['animName'] = 'throw-paper'
        return doThrowGear(attack)
    elif suitName == 'bf':
        attack['id'] = CANNED
        attack['name'] = 'Canned'
        attack['animName'] = 'throw-paper'
        ...
        attack['id'] = MUD_SLING
        attack['name'] = 'MudSling'
        attack['animName'] = 'throw-object'
        return doMudSling(attack)
    elif suitName == 'ac':
        attack['id'] = RED_TAPE
        attack['name'] = 'RedTape'
        attack['animName'] = 'throw-object'
        ...
        attack['id'] = COUNTERCLAIM
        attack['name'] = 'Counterclaim'
        attack['animName'] = 'throw-paper'
        return doCounterclaim(attack)
    elif suitName == 'wsi':
        attack['id'] = GUILT_TRIP_WSI
        attack['name'] = 'GuiltTripWSI'
        attack['animName'] = 'magic1'
        return doGuiltTripWSI(attack)
    elif suitName == 'sc':
        ...
        attack['id'] = CASH_RAIN
        attack['name'] = 'CashRain'
        attack['animName'] = 'magic1'
        return doCashRain(attack)
    elif suitName == 'rb':
        ...
        attack['id'] = ACID_RAIN
        attack['name'] = 'AcidRain'
        attack['animName'] = 'magic1'
        return doAcidRain(attack)
    elif suitName == 'dpr':
        attack['id'] = GLOWER_POWER
        attack['name'] = 'GlowerPower'
        attack['animName'] = 'glower'
...
def doPowerTie(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay, ['sidestep'])
    throwSound = getSoundTrack('SA_powertie_throw.ogg', delay=soundDelay, node=suit)
    if dmg > 0:
        hitSound = getSoundTrack('SA_powertie_impact.ogg', delay=soundDelay + 0.6, node=suit)
...
def doPoundKey(attack):
    ...
    if suitName == 'cc':
        phone = globalPropPool.getProp('cold-caller-phone')
        receiver = globalPropPool.getProp('cold-caller-receiver')
    else:
        phone = globalPropPool.getProp('phone')
        receiver = globalPropPool.getProp('receiver')
    suitType = getSuitBodyType(attack['suitName'])
    ...
    if suitType == 'c':
        propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)), Func(__showProp, receiver, suit.getLeftHand(), Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(0.74), Func(receiver.wrtReparentTo, suit.getRightHand()), LerpPosHprInterval(receiver, 0.0001, Point3(-0.45, 0.48, -0.62), VBase3(-87.47, -18.21, 7.82)), Wait(3.14), Func(receiver.wrtReparentTo, phone), Wait(0.62), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [receiver, phone]))
    else:
...
def doHangUp(attack):
    ...
    if suitName == 'tf':
        ...
        phonePosPoints = [Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)]
        receiverPosPoints = [Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)]
        receiverAdjustScale = Point3(-0.45, 0.48, -0.62)
        pickupDelay = 0.74
        dialDuration = 3.07
        finalPhoneDelay = 0.01
        scaleUpPoint = MovieUtil.PNT3_ONE
    ...
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'c':
        propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(phone, 0.5, scaleUpPoint, MovieUtil.PNT3_NEARZERO), Wait(pickupDelay), Func(receiver.wrtReparentTo, suit.getRightHand()), LerpPosHprInterval(receiver, 0.0001, Point3(-0.45, 0.48, -0.62), VBase3(-87.47, -18.21, 7.82)), Wait(3.14), Func(receiver.wrtReparentTo, phone), Wait(0.62), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [receiver, phone]))
    else:
        propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(phone, 0.5, scaleUpPoint, MovieUtil.PNT3_NEARZERO), Wait(pickupDelay), Func(receiver.wrtReparentTo, suit.getRightHand()), LerpScaleInterval(receiver, 0.01, receiverAdjustScale), LerpPosHprInterval(receiver, 0.0001, Point3(-0.53, 0.21, -0.54), VBase3(-99.49, -35.27, 1.84)), Wait(dialDuration), Func(receiver.wrtReparentTo, phone), Wait(finalPhoneDelay), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [receiver, phone]))
...
def doFillWithLead(attack):
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack('SA_fill_with_lead.ogg', delay=0.5, node=suit)
        return Parallel(suitTrack, pencilPropTrack, sharpenerPropTrack, sprayTrack, headTrack, torsoTrack, legsTrack, colorTrack, toonTrack, soundTrack)
    else:
        soundTrack = getSoundTrack('SA_fill_with_lead_miss.ogg', delay=0.5, node=suit)
        return Parallel(suitTrack, pencilPropTrack, sharpenerPropTrack, sprayTrack, toonTrack, soundTrack)
...
def doFingerWag(attack):
    ...
    particleEffect2 = BattleParticles.createParticleEffect(file='fingerwag2')
    BattleParticles.setEffectTexture(particleEffect, 'blah', color=Vec4(0.55, 0, 0.55, 1))
    BattleParticles.setEffectTexture(particleEffect2, 'blah', color=Vec4(0.55, 0, 0.55, 1))
    suitType = getSuitBodyType(attack['suitName'])
    partDelay = 1.3
    damageDelay = 2.7
    if suitType == 'a':
    ...
    partTrack2 = getPartTrack(particleEffect2, partDelay, 2, [particleEffect2, suit, 0])
    suitName = attack['suitName']
    if suitName == 'mm':
        particleEffect.setPos(0.167, 1.5, 2.731)
        particleEffect2.setPos(0.167, 1.5, 2.731)
    elif suitName == 'tw':
        particleEffect.setPos(0.167, 1.8, 5)
        particleEffect.setHpr(-90.0, -60.0, 180.0)
        particleEffect2.setPos(0.167, 1.8, 5)
    elif suitName == 'pp':
        particleEffect.setPos(0.167, 1, 4.1)
        particleEffect2.setPos(0.167, 1, 4.1)
    elif suitName == 'bs':
        particleEffect.setPos(0.167, 1, 5.1)
        particleEffect2.setPos(0.167, 1, 5.1)
    elif suitName == 'le':
        particleEffect.setPos(0.167, 1.9, suit.getHeight() - 1.8)
        particleEffect.setP(-110)
        particleEffect2.setPos(0.167, 1.9, suit.getHeight() - 1.8)
    elif suitName == 'bw':
        particleEffect.setPos(0.167, 1.9, suit.getHeight() - 1.8)
        particleEffect.setP(-110)
        particleEffect2.setPos(0.167, 1.9, suit.getHeight() - 1.8)
...
def doRubberStamp(attack):
    ...
    if suitType == 'a':
        ...
        padPosPoints = [Point3(-0.65, 0.83, -0.04), VBase3(5.625, 4.456, -165.125)]
        stampPosPoints = [Point3(-0.64, -0.17, -0.03), MovieUtil.PNT3_ZERO]
    padPropTrack = getPropTrack(globalPropPool.getProp('pad'), suit.getLeftHand(), padPosPoints, 1e-06, 3.2)
    ...
    if dmg > 0:
        soundTrack = getSoundTrack('SA_rubber_stamp.ogg', delay=1.3, node=suit)
    else:
...
def doFloodTheMarket(attack):
    ...
    dodgeAnims.extend(getSplicedLerpAnims('jump', 0.31, 1.3, startTime=0.6))
    dodgeAnims.append(['jump', 0, 0.91])
    toonTracks = getToonTracks(attack, damageDelay=0.7, splicedDamageAnims=[['melt'], ['jump', 1.5, 0.4]], dodgeDelay=0.91, splicedDodgeAnims=dodgeAnims, showMissedExtraTime=1.0)
    synergySoundTrack = Sequence(Wait(0.9), SoundInterval(globalBattleSoundCache.getSound('SA_extra_tip.ogg'), node=suit))
    return Parallel(suitTrack, partTrack, waterfallTrack, synergySoundTrack, toonTracks)
...
def doSnowWave(attack):
    ...
    particleEffect = BattleParticles.createParticleEffect(file='snowWave')
    waterfallEffect = BattleParticles.createParticleEffect(file='snowWaveWaterfall')
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect, 1.0, 1.9, [particleEffect, suit, 0])
    waterfallTrack = getPartTrack(waterfallEffect, 0.8, 1.9, [waterfallEffect, suit, 0])
    damageAnims = [['slip-forward']]
    ...
    dodgeAnims.append(['jump', 0, 0.91])
    toonTracks = getToonTracks(attack, damageDelay=0.7, splicedDamageAnims=[['melt'], ['jump', 1.5, 0.4]], dodgeDelay=0.91, splicedDodgeAnims=dodgeAnims, showMissedExtraTime=1.0)
    return Parallel(suitTrack, partTrack, waterfallTrack, toonTracks)


def doColdHardCash(attack):
    suit = attack['suit']
    battle = attack['battle']
    targets = attack['target']
    damageDelay = 1.7
    ...
    particleEffect = BattleParticles.createParticleEffect(file='coldHardCashMoney')
    waterfallEffect = BattleParticles.createParticleEffect(file='coldHardCashMoneyWaterfall')
    particleEffect2 = BattleParticles.createParticleEffect(file='coldHardCashSnow')
    waterfallEffect2 = BattleParticles.createParticleEffect(file='coldHardCashSnowWaterfall')
    particleEffect3 = BattleParticles.createParticleEffect(file='coldHardCashTwinkle')
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect, 1.0, 1.9, [particleEffect, suit, 0])
    waterfallTrack = getPartTrack(waterfallEffect, 0.8, 1.9, [waterfallEffect, suit, 0])
    partTrack2 = getPartTrack(particleEffect2, 1.0, 1.9, [particleEffect2, suit, 0])
    waterfallTrack2 = getPartTrack(waterfallEffect2, 0.8, 1.9, [waterfallEffect2, suit, 0])
    partTrack3 = getPartTrack(particleEffect3, 1.0, 1.9, [particleEffect3, suit, 0])
    dodgeAnims = []
    dodgeAnims.append(['jump',
     0.01,
     0,
    ...
    magicSound = Sequence(Wait(0.9), SoundInterval(globalBattleSoundCache.getSound('SA_withdrawl.ogg'), node=suit))
    freezeSound = Sequence(Wait(0.9), SoundInterval(globalBattleSoundCache.getSound('SA_freeze_assets_trim.ogg'), node=suit))
    moneySound = Sequence(Wait(0.9), SoundInterval(globalBattleSoundCache.getSound('SA_multi_level_marketing.ogg'), node=suit))
    return Parallel(suitTrack, partTrack, partTrack2, partTrack3, waterfallTrack, waterfallTrack2, magicSound, freezeSound, moneySound, toonTracks)
...
def doRainOfFire(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    BattleParticles.loadParticles()
    rainEffect = BattleParticles.createParticleEffect(file='rainOfFire')
    rainEffect2 = BattleParticles.createParticleEffect(file='rainOfFire')
    rainEffect3 = BattleParticles.createParticleEffect(file='rainOfFire')
    BattleParticles.setEffectTexture(rainEffect, 'fire')
    cloud = globalPropPool.getProp('stormcloud')
    partDelay = 0.2
    damageDelay = 3.5
    dodgeDelay = 2.3
...
def doFreezeAssets(attack):
    ...
    damageAnims = [['cringe',
      ...
      0.8], ['duck', 0.01, 1.6]]
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay, dodgeAnimNames=['sidestep'], showMissedExtraTime=1.2)
    soundTrack = getSoundTrack('SA_freeze_assets.ogg', delay=2.0, node=suit)
    return Parallel(suitTrack, toonTrack, cloudPropTrack, soundTrack)
...
def doSandTrap(attack):
    ...
    if dmg > 0:
        puddleTrack = Sequence(Func(battle.movie.needRestoreRenderProp, puddle), Wait(damageDelay - 0.7), Func(puddle.reparentTo, battle), Func(puddle.setPos, toon.getPos(battle)), LerpScaleInterval(puddle, 1.7, Point3(1.7, 1.7, 1.7), startScale=MovieUtil.PNT3_NEARZERO), Wait(3.2), LerpFunctionInterval(puddle.setAlphaScale, fromData=1, toData=0, duration=0.8), Func(MovieUtil.removeProp, puddle), Func(battle.movie.clearRenderProp, puddle))
        soundTrack = getSoundTrack('TL_quicksand.ogg', delay=0.5, node=suit)
        return Parallel(suitTrack, toonTrack, soundTrack, puddleTrack)
    else:
        puddleTrack = Sequence(Func(battle.movie.needRestoreRenderProp, puddle), Wait(damageDelay - 0.7), Func(puddle.reparentTo, battle), Func(puddle.setPos, toon.getPos(battle)), LerpScaleInterval(puddle, 1.7, Point3(1.7, 1.7, 1.7), startScale=MovieUtil.PNT3_NEARZERO), Wait(0.3), LerpFunctionInterval(puddle.setAlphaScale, fromData=1, toData=0, duration=0.8), Func(MovieUtil.removeProp, puddle), Func(battle.movie.clearRenderProp, puddle))
        soundTrack = getSoundTrack('TL_quicksand.ogg', duration=0.67, delay=0.5, node=suit)
        return Parallel(suitTrack, toonTrack, soundTrack, puddleTrack)
...
def doTeeOff(attack):
    ...
    suitTrack = getSuitTrack(attack)
    clubPosPoints = [Point3(0.0, 0.0, 0.0), VBase3(63.097, 43.988, -18.435)]
    ...
    if suitName == 'ym':
        ...
        ballPosPoints = [Point3(3.3, 0, 0.1)]
    else:
        ballPosPoints = [Point3(2.1, 0, 0.1)]
    ...
    if dmg > 0:
        ...
        mulligan = Sequence(doMulligan(attack))
        notify.debug('Tee Off time taken: %s' % suitTrack.getDuration)
        return Parallel(suitTrack, toonTrack, clubPropTrack, ballPropTrack, soundTrack, mulligan)
...
def doCigarSmoke(attack):
    ...
    smoke = BattleParticles.createParticleEffect(file='smokeTest4')
    suitName = attack['suitName']
    if suitName == 'hho':
        smoke.setPos(0.0, 3.25, 7.0)
        smoke.setHpr(0.0, -135.0, 0.0)
    elif suitName == 'hoo':
        smoke.setPos(0.0, 3.25, 7.0)
        smoke.setHpr(0.0, -135.0, 0.0)
    elif suitName == 'tl':
        ...
        smoke.setPos(0.0, 0.0, 1.0)
        smoke.setHpr(0.0, -90.0, 0.0)
    smokeTrack = getPartTrack(smoke, 3.4, 1.5, [smoke, suit, 0])
    BattleParticles.loadParticles()
    suitTrack = getSuitTrack(attack)
    ...
    if dmg > 0:
        ...
        colorTrack.append(Func(battle.movie.clearRestoreColor))
        return Parallel(suitTrack, toonTrack, cigarPropTrack, smokeTrack, colorTrack)
    else:
...
def doBrainStorm(attack):
    ...
    suitName = suit.getStyleName
    if suitName == 'da':
        peckingOrderInstead = doPeckingOrder(attack)
        return Parallel(peckingOrderInstead)
    else:
        battle = attack['battle']
        target = attack['target']
        toon = target['toon']
        ...
        cloudPropTrack.append(Wait(1.1))
        cloudPropTrack.append(LerpPosInterval(cloud, 1, pos=targetPoint))
        cloudPropTrack.append(Wait(1.2))
...
def doDownPayment(attack):
    ...
    dmg = target['hp']
    BattleParticles.loadParticles()
    snowEffect = BattleParticles.createParticleEffect(file='downPayment')
    snowEffect2 = BattleParticles.createParticleEffect(file='downPayment')
    snowEffect3 = BattleParticles.createParticleEffect(file='downPayment')
    BattleParticles.setEffectTexture(snowEffect, 'dollar-sign', color=Vec4(0.00, 1.00, 0.00, 1.00))
    BattleParticles.setEffectTexture(snowEffect2, 'dollar-sign', color=Vec4(0.00, 1.00, 0.00, 1.00))
    BattleParticles.setEffectTexture(snowEffect3, 'dollar-sign', color=Vec4(0.00, 1.00, 0.00, 1.00))
    toonPos = toon.getPos(toon)
    cloudPos = Point3(toonPos.getX(), toonPos.getY(), toonPos.getZ() + toon.getHeight() + 0.55)
    snowEffect.setPos(cloudPos)
    snowEffect2.setPos(cloudPos)
    snowEffect3.setPos(cloudPos)
    suitTrack = getSuitTrack(attack)
    cloudTrack = getPartTrack(snowEffect, 0.0, 1.9, [snowEffect, toon, 0])
    cloudTrack2 = getPartTrack(snowEffect, 0.0, 1.9, [snowEffect2, toon, 0])
    cloudTrack3 = getPartTrack(snowEffect, 0.0, 1.9, [snowEffect3, toon, 0])
    targetPoint = __toonFacePoint(toon)
    targetPoint.setZ(targetPoint[2] + 3)
    if dmg > 0:
        initialScale = toon.getScale()
        downScale = Vec3(0.4, 0.4, 0.4)
        shrinkTrack = Sequence(Func(battle.movie.needRestoreToonScale), LerpScaleInterval(toon, 1.0, downScale * 1.1), LerpScaleInterval(toon, 0.1, downScale * 0.9), LerpScaleInterval(toon, 0.1, downScale * 1.05), LerpScaleInterval(toon, 0.1, downScale * 0.95), LerpScaleInterval(toon, 0.1, downScale), Wait(2.1), LerpScaleInterval(toon, 0.5, initialScale * 1.5), LerpScaleInterval(toon, 0.15, initialScale * 0.5), LerpScaleInterval(toon, 0.15, initialScale * 1.2), LerpScaleInterval(toon, 0.15, initialScale * 0.8), LerpScaleInterval(toon, 0.15, initialScale), Func(battle.movie.clearRestoreToonScale))
    damageAnims = []
    ...
    damageAnims.append(['sidestep-right',
     ...
     2.97,
     1.49])
    toonTrack = getToonTrack(attack, damageDelay=0.0, splicedDamageAnims=damageAnims, dodgeDelay=0.0, dodgeAnimNames=['sidestep'])
    soundTrack = getSoundTrack('SA_filibuster.ogg', node=toon)
    if dmg > 0:
        growSound = getSoundTrack('SA_head_grow_back_only.ogg', delay=3.9, node=toon)
        return Parallel(suitTrack, cloudTrack, cloudTrack2, cloudTrack3, shrinkTrack, toonTrack, soundTrack, growSound)
    else:
        return Parallel(suitTrack, cloudTrack, cloudTrack2, cloudTrack3, toonTrack, soundTrack)
...
def doBrainFreeze(attack):
    ...
    dmg = target['hp']
    BattleParticles.loadParticles()
    sprayEffect = BattleParticles.createParticleEffect(file='brainFreezeSpray')
    freezeEffect = BattleParticles.createParticleEffect(file='brainFreezeSmother')
    BattleParticles.setEffectTexture(sprayEffect, 'snow-particle')
    BattleParticles.setEffectTexture(freezeEffect, 'snow-particle')
    facePoint = __toonFacePoint(toon)
    freezeEffect.setPos(0, 0, facePoint.getZ())
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(sprayEffect, 0.4, 1.1, [sprayEffect, suit, 0])
    partTrack2 = getPartTrack(freezeEffect, 1.4, 2.9, [freezeEffect, toon, 0])
    toonTrack = getToonTrack(attack, damageDelay=1.0, damageAnimNames=['cringe'], dodgeDelay=0.0, dodgeAnimNames=['duck'], showMissedExtraTime=1.1, showDamageExtraTime=1.1)
    headTrack = getPartTrack(freezeEffect, 1.1, 1.4, [freezeEffect, toon, 0])
    soundTrack = getSoundTrack('SZ_TB_wind_1.ogg', delay=0.7, node=suit)
    soundTrack2 = getSoundTrack('SZ_TB_wind_2.ogg', delay=0.7, node=suit)
    soundTrack3 = getSoundTrack('SZ_TB_wind_3.ogg', delay=0.7, node=suit)

    def colorParts(parts):
        track = Parallel()
        for partNum in xrange(0, parts.getNumPaths()):
            nextPart = parts.getPath(partNum)
            track.append(Func(nextPart.setColorScale, Vec4(0, 0.5, 1.0, 1)))
    ...
    if dmg > 0:
        colorTrack = Sequence()
        headParts = toon.getHeadParts()
        colorTrack.append(Wait(1.1))
        colorTrack.append(Func(battle.movie.needRestoreColor))
        colorTrack.append(colorParts(headParts))
        colorTrack.append(Wait(2.5))
        colorTrack.append(resetParts(headParts))
        colorTrack.append(Func(battle.movie.clearRestoreColor))
        freezeSound = getSoundTrack('tt_s_ara_tbr_trashcan_idleShiver1.ogg', delay=1.1, node=toon)
        freezeSound2 = getSoundTrack('tt_s_ara_tbr_mailbox_idleShiver1.ogg', delay=1.1, node=toon)
        return Parallel(suitTrack, toonTrack, soundTrack, soundTrack2, soundTrack3, freezeSound, freezeSound2, partTrack, partTrack2, headTrack, colorTrack)
    else:
        return Parallel(suitTrack, toonTrack, soundTrack, soundTrack2, soundTrack3, partTrack)
...
def doDownsize(attack):
    ...
    if dmg > 0:
        soundTrack = getSoundTrack('SA_downsize.ogg', delay=0.3, node=suit)
        return Parallel(suitTrack, sprayTrack, cloudTrack, shrinkTrack, toonTrack, soundTrack)
    else:
        soundTrack = getSoundTrack('SA_downsize_miss.ogg', delay=0.3, node=suit)
...
def doPinkSlip(attack):
    ...
    soundTrack = getSoundTrack('SA_pink_slip.ogg', delay=1.0, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)
...
def doReOrg(attack):
    ...
    cloudEffect = BattleParticles.createParticleEffect(file='reorgCloud')
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(sprayEffect, 1.0, 1.9, [sprayEffect, suit, 0])
    partTrack2 = getPartTrack(cloudEffect, 1.9, 2.4, [cloudEffect, toon, 0])
    if dmg > 0:
        headParts = toon.getHeadParts()
    ...
    if dmg > 0:
        return Parallel(suitTrack, partTrack, partTrack2, toonTrack, headTracks, chestTracks)
    else:
...
def doSacked(attack):
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack('SA_sacked.ogg', delay=1.0, node=suit)
    else:
        ...
        soundTrack = getSoundTrack('SA_sacked_miss.ogg', delay=1.0, node=suit)
    damageAnims = [['struggle',
      0.01,
      0.01,
      0.7], ['slip-backward', 0.01, 0.45]]
    ...
    return Parallel(suitTrack, toonTrack, sackTrack, soundTrack)
...
def doHalfWindsor(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay, dodgeAnimNames=['sidestep'])
    throwSound = getSoundTrack('SA_powertie_throw.ogg', delay=throwDelay + 1, node=suit)
    return Parallel(suitTrack, toonTrack, throwSound, tiePropTrack)
...
def doRolodex(attack):
    ...
    vortexEffect = BattleParticles.createParticleEffect(file='rollodexVortex')
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
    ...
    vortexTrack = getPartTrack(vortexEffect, 2.8, partDuration, [vortexEffect, suit, 0])
    suitTrack = getSuitTrack(attack)
    ...
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack, partTrack2, partTrack3, vortexTrack)
...
def doEvilEye(attack):
    ...
    if suitName == 'cr':
        ...
        posPoints = [Point3(-0.64, 4.45, 5.01), VBase3(-155.0, -20.0, 0.0)]
    elif suitName == 'le':
...
def doJump(attack):
    suit = attack['suit']
    suitSplicedAnims = []
    suitSplicedAnims.append(['quick-jump',
     0.01,
     0.01,
     18])
    suitTrack = getSplicedAnimsTrack(attack, ['quick-jump', 0, 18, 18, 20, 97, 111, 112, 138])
    ...
    toonTrack = getToonTrack(attack, 1.8, ['slip-forward'], 1.1, ['jump'])
    return Parallel(suitTrack, toonTracks)


def doEvilEyeWSI(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
...
def doBlackball(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    dmg = target['hp']
    ball = globalPropPool.getProp('blackball')
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        suitDelay = 1.79
        damageDelay = 3.46
...
def doDoubleTalk(attack):
    ...
    soundTrack2 = getSoundTrack('SA_doubletalk.ogg', delay=2.5, node=suit)
    return Parallel(suitTrack, toonTrack, partTrack, partTrack2, soundTrack, soundTrack2)
...
def doSchmooze(attack):
    ...
    for i in xrange(0, 4):
        ...
        BattleParticles.setEffectTexture(upperEffect, textureNames[i], color=Vec4(0.4, 0, 0, 1))
        BattleParticles.setEffectTexture(lowerEffect, textureNames[i], color=Vec4(0.4, 0, 0, 1))
    ...
    soundTrack = getSoundTrack('SA_schmooze.ogg', delay=partDelay, node=suit)
    suitName = suit.getStyleName()
    if suitName == 'dpr':
        glowerPower = doGlowerPower(attack)
...
def doGuiltTrip(attack):
    ...
    targets = attack['target']
    tauntIndex = attack['taunt']
    # TODO: Give the Witness Stand-in a special taunt index different from that of a normal Cog.
    suitName = suit.getStyleName()
    if suitName == 'wsi':
        taunt = getAttackTaunt(attack['name'], ['Have you ever considered how you make US feel?', 'You\'ve exhausted me, Toon... I haven\'t had time to rest with you "Breaking the Law".', "I've lost every ounce of sympathy for you Toons.", 'You have caused nothing but anguish for me!', 'There is so much more beyond the surface!'])
    else:
        taunt = getAttackTaunt(attack['name'], tauntIndex)
    hitAtleastOneToon = 0
    for t in targets:
    ...
    if hitAtleastOneToon == 1:
        for t in targets:
            toon = t['toon']
        getUpSound = getSoundTrack('Toon_getting_up_guilt_trip.ogg', delay=2.7, node=toon)
        return Parallel(suitTrack, partTrack1, partTrack2, soundTrack, waterfallTrack, toonTracks, getUpSound)
    else:
        return Parallel(suitTrack, partTrack1, partTrack2, soundTrack, waterfallTrack, toonTracks)


def doGuiltTripWSI(attack):
    suit = attack['suit']
    battle = attack['battle']
    centerColor = Vec4(1.0, 0.2, 0.2, 0.9)
    edgeColor = Vec4(0.9, 0.9, 0.9, 0.4)
...
def doFired(attack):
    ...
    flameEffect = BattleParticles.createParticleEffect('FiredFlame')
    flecksEffect = BattleParticles.createParticleEffect('SpriteFiredFlecks')
    smokeEffect = BattleParticles.createParticleEffect(file='smokeTest4')
    BattleParticles.setEffectTexture(baseFlameEffect, 'fire')
    BattleParticles.setEffectTexture(flameEffect, 'fire')
    ...
    smokeSmall = BattleParticles.createParticleEffect(file='smokeTest4')
    ...
    smokeTrack = getPartTrack(smokeEffect, 1.0, 1.9, [smokeEffect, toon, 0])
    ...
    smokeSmallTrack = getPartTrack(smokeSmall, 1.0, 1.9, [smokeSmall, toon, 0])
    ...
    if dmg > 0:
        return Parallel(suitTrack, baseFlameTrack, flameTrack, flecksTrack, smokeTrack, toonTrack, colorTrack, soundTrack)
    else:
        return Parallel(suitTrack, baseFlameSmallTrack, flameSmallTrack, flecksSmallTrack, smokeSmallTrack, toonTrack, soundTrack)
...
def doCrunch(attack):
    ...
    if dmg > 0:
        soundTrack = getSoundTrack('SA_crunch.ogg', delay=0.8, node=suit)
    else:
        soundTrack = getSoundTrack('SA_crunch_miss.ogg', delay=0.8, node=suit)
    return Parallel(suitTrack, toonTrack, numberSpillTrack1, numberSpillTrack2, numberTracks, numberSprayTracks, soundTrack)


def doMudSling(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    ...
    numberNames = ['raindrop',
     ...
     'raindrop']
    BattleParticles.loadParticles()
    numberSpill1 = BattleParticles.createParticleEffect(file='mudSlingSpill')
    numberSpill2 = BattleParticles.createParticleEffect(file='mudSlingSpill')
    spillTexture1 = random.choice(numberNames)
    spillTexture2 = random.choice(numberNames)
    BattleParticles.setEffectTexture(numberSpill1, 'raindrop')
    BattleParticles.setEffectTexture(numberSpill2, 'raindrop')
    numberSpillTrack1 = getPartTrack(numberSpill1, 1.1, 2.2, [numberSpill1, suit, 0])
    numberSpillTrack2 = getPartTrack(numberSpill2, 1.5, 1.0, [numberSpill2, suit, 0])
    numberSprayTracks = Parallel()
    numOfNumbers = random.randint(5, 9)
    for i in xrange(0, numOfNumbers - 1):
        nextSpray = BattleParticles.createParticleEffect(file='mudSlingSpray')
        nextTexture = random.choice(numberNames)
        BattleParticles.setEffectTexture(nextSpray, 'raindrop')
        nextStartTime = random.random() * 0.6 + throwDuration
        nextDuration = random.random() * 0.4 + 1.4
        nextSprayTrack = getPartTrack(nextSpray, nextStartTime, nextDuration, [nextSpray, suit, 0])
        numberSprayTracks.append(nextSprayTrack)
    ...
    for i in xrange(0, numOfNumbers):
        texture = random.choice(numberNames)
        next = MovieUtil.copyProp(BattleParticles.getParticle('raindrop'))
        next.reparentTo(suit.getRightHand())
        next.setScale(0.01, 0.01, 0.01)
        next.setColor(Vec4(0.5, 0.25, 0.0, 1.0))
        next.setPos(random.random() * 0.6 - 0.3, random.random() * 0.6 - 0.3, random.random() * 0.6 - 0.3)
        next.setHpr(VBase3(-1.15, 86.58, -76.78))
        numberTrack = Sequence(Wait(0.9), LerpScaleInterval(next, 0.6, MovieUtil.PNT3_ONE), Wait(1.7), Func(MovieUtil.removeProp, next))
        numberTracks.append(numberTrack)
    ...
    damageAnims.append(['slip-forward', 0.01, 0.6])
    toonTrack = getToonTrack(attack, damageDelay=4.7, splicedDamageAnims=damageAnims, dodgeDelay=3.6, dodgeAnimNames=['sidestep'])
    soundTrack = getSoundTrack('SA_watercooler_spray_only.ogg', delay=4.7, node=toon)
    return Parallel(suitTrack, toonTrack, numberSpillTrack1, numberSpillTrack2, numberTracks, numberSprayTracks, soundTrack)
...
def doMarketCrash(attack):
    ...
    soundTrack = getSoundTrack('SA_market_crash.ogg', delay=0.3, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)
...
def doBite(attack):
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack('SA_bite.ogg', delay=propDelay, node=suit)
    else:
        ...
        soundTrack = getSoundTrack('SA_bite_miss.ogg', delay=propDelay, node=suit)
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        animTrack = Sequence(Wait(throwDelay), ActorInterval(teeth, 'teeth', duration=throwDuration), ActorInterval(teeth, 'teeth', duration=0.3), Func(teeth.pose, 'teeth', 1), Wait(0.7), ActorInterval(teeth, 'teeth', duration=0.9))
        propTrack = Sequence(Parallel(teethAppearTrack, scaleTrack, hprTrack, animTrack), Func(MovieUtil.removeProp, teeth), Func(battle.movie.clearRenderProp, teeth))
        soundTrack = getSoundTrack('SA_chomp.ogg', node=suit)
    else:
        x = toon.getX(battle)
        y = toon.getY(battle)
        z = toon.getZ(battle)
        ...
        soundTrack = getSoundTrack('SA_chomp.ogg', duration=2.6, node=suit)
    damageAnims = [['cringe',
      0.01,
      0.7,
      1.2],
...
def doEvictionNotice(attack):
    ...
    soundTrack = getSoundTrack('SA_eviction_notice.ogg', node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)


def doThrowGear(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    gear = globalPropPool.getProp('gear')
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(0.5, 0.0, 0.0), VBase3(0.0, 270.0, 0.0)]
    propTrack = Sequence(getPropAppearTrack(gear, suit.getRightHand(), posPoints, 0.5, scaleUpPoint=Point3(0.1, 0.1, 0.1), scaleUpTime=0.5))
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a' or suitType == 'b':
        propTrack.append(Wait(2.0))
        damageDelay = 3.4
        dodgeDelay = 2.8
        soundDelay = 2.9
    elif suitType == 'c':
    ...
    missPoint = __toonGroundPoint(attack, toon, 0.7, parent=battle)
    missPoint.setX(missPoint.getX() - 1.1)
    propTrack.append(getPropThrowTrack(attack, gear, [hitPoint], [missPoint], parent=battle))
    toonTrack = getToonTrack(attack, damageDelay, ['slip-backward'], dodgeDelay, ['sidestep'])
    soundTrack = getSoundTrack('CHQ_VP_frisbee_gears.ogg', delay=soundDelay, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)
...
def doInkDrain(attack):
    ...
    soundTrack = getSoundTrack('SA_ink_drain.ogg', node=suit)
    if dmg > 0:
        colorTrack = Sequence()
        colorTrack.append(Wait(1.6))
        colorTrack.append(Func(battle.movie.needRestoreColor))
...
def doBloodMoney(attack):
    ...
    dmg = target['hp']
    BattleParticles.loadParticles()
    particleEffect = BattleParticles.createParticleEffect(file='bloodMoney')
    BattleParticles.setEffectTexture(particleEffect, 'dollar-sign')
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect, 1e-05, suitTrack.getDuration() + 1.2, [particleEffect, suit, 0])
    toonTrack = getToonTrack(attack, 1.2, ['cringe'], 0.2, splicedDodgeAnims=[['duck', 1e-05, 0.8]], showMissedExtraTime=0.8)
    soundTrack = getSoundTrack('SA_ink_drain.ogg', node=suit)
...
def doPositiveReinforcement(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    ...
    dodgeDelay = 0.5
    suitTrack = getSuitAnimTrack(attack)
    toonTrack = getToonTrack(attack, 1.2, ['cringe'], 0.2, splicedDodgeAnims=[['duck', 0.0]])
    headParts = toon.getHeadParts()
    torsoParts = toon.getTorsoParts()
    legsParts = toon.getLegsParts()
    ...
    def changeColor(parts):
        ...
        for partNum in xrange(0, parts.getNumPaths()):
            nextPart = parts.getPath(partNum)
            track.append(Func(nextPart.setColorScale, Vec4(0, 1, 0, 1)))
    ...
    if dmg > 0:
        ...
        colorTrack.append(resetColor(legsParts))
        colorTrack.append(Func(battle.movie.clearRestoreColor))
        return Parallel(suitTrack, toonTrack, colorTrack)
    else:
        return Parallel(suitTrack, toonTrack)
...
def doRestrainingOrder(attack):
    ...
    toon = target['toon']
    dmg = target['hp']
    paper = globalPropPool.getProp('restraining-order')
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(-0.04, 0.15, -1.38), VBase3(10.584, -11.945, 18.316)]
    propTrack = Sequence(getPropAppearTrack(paper, suit.getRightHand(), posPoints, 0.8, MovieUtil.PNT3_ONE, scaleUpTime=0.5))
    propTrack.append(Wait(1.73))
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack('SA_restraining_order_miss.ogg', node=suit)
        return Parallel(suitTrack, toonTrack, propTrack, soundTrack)


def doRestrainingOrderWSI(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
...
def doCounterclaim(attack):
    ...
    missPoint.setX(missPoint.getX() - 1.1)
    propTrack.append(getPropThrowTrack(attack, paper, [hitPoint], [missPoint], parent=battle))
    toonTrack = getToonTrack(attack, 3.4, ['conked'], 2.8, ['sidestep'])
    soundTrack = getSoundTrack('SA_eviction_notice.ogg', node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)


def doBlacklist(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    paper = globalPropPool.getProp('blacklist')
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(-0.04, 0.15, -1.38), VBase3(10.584, -11.945, 18.316)]
    propTrack = Sequence(getPropAppearTrack(paper, suit.getRightHand(), posPoints, 0.8, MovieUtil.PNT3_ONE, scaleUpTime=0.5))
    propTrack.append(Wait(1.73))
...
def doSpin(attack):
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack('SA_spin_miss.ogg', delay=1.0, node=suit)
        return Parallel(suitTrack, sprayTrack, toonTrack, soundTrack)


def doMarketingSpin(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    dmg = target['hp']
    damageDelay = 1.7
    sprayEffect = BattleParticles.createParticleEffect(file='marketingSpinSpray')
    spinEffect1 = BattleParticles.createParticleEffect(file='marketingSpinEffect')
    spinEffect2 = BattleParticles.createParticleEffect(file='marketingSpinEffect')
    spinEffect3 = BattleParticles.createParticleEffect(file='marketingSpinEffect')
    spinEffect1.reparentTo(toon)
    spinEffect2.reparentTo(toon)
    spinEffect3.reparentTo(toon)
    height1 = toon.getHeight() * (random.random() * 0.2 + 0.7)
    ...
    damageAnims.extend(getSplicedLerpAnims('think', 0.66, 1.1, startTime=2.26))
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=0.91, dodgeAnimNames=['sidestep'], showDamageExtraTime=2.1, showMissedExtraTime=1.0)
    soundTrack = getSoundTrack('SA_life_insurance_register.ogg', delay=1.0, node=suit)
    if dmg > 0:
        toonSpinTrack = Sequence(Wait(damageDelay + 0.9), LerpHprInterval(toon, 0.7, Point3(-10, 0, 0)), LerpHprInterval(toon, 0.5, Point3(-30, 0, 0)), LerpHprInterval(toon, 0.2, Point3(-60, 0, 0)), LerpHprInterval(toon, 0.7, Point3(-700, 0, 0)), LerpHprInterval(toon, 1.0, Point3(-1310, 0, 0)), LerpHprInterval(toon, 0.4, toon.getHpr()), Wait(0.5))
        return Parallel(suitTrack, sprayTrack, toonTrack, toonSpinTrack, spinTrack1, spinTrack2, spinTrack3, soundTrack)
    else:
...
def doLegalese(attack):
    ...
    if dmg > 0:
        ...
        return Parallel(suitTrack, toonTrack, sprayTrack1, sprayTrack2, sprayTrack3, soundTrack)
    else:
...
def doPeckingOrder(attack):
    ...
    if dmg > 0:
        soundTrack = getSoundTrack('SA_pecking_order.ogg', delay=2.1, node=suit)
    else:
        soundTrack = getSoundTrack('SA_pecking_order_miss.ogg', delay=2.1, node=suit)
    return Parallel(suitTrack, toonTrack, birdTracks, soundTrack)


def doPeckingOrderWSI(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
...
def doEvidence(attack):
    ...
    paper = globalPropPool.getProp('lawbook')
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(0, 0.5, -0.3), VBase3(0, 0, 0)]
...
def doCigar(attack):
    ...
    dmg = target['hp']
    suitType = getSuitBodyType(attack['suitName'])
    particleEffect = BattleParticles.createParticleEffect('Smoke')
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(particleEffect, 3.5, 1.9, [particleEffect, suit, 0])
    damageDelay = 3
    dodgeDelay = 1
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay, ['duck'])
    throwSound = getSoundTrack('SA_filibuster.ogg', delay=1, node=suit)
    return Parallel(suitTrack, partTrack, toonTrack, throwSound)
...
def doSongAndDance(attack):
    ...
    dmg = target['hp']
    suitTrack = getSuitTrack(attack)
    toonTrack = getToonTrack(attack, 3.9, ['cringe'], 3.9, ['applause'])
    if dmg > 0:
        soundTrack = getSoundTrack('AA_heal_happydance.ogg', duration=4.0, node=suit)
    else:
