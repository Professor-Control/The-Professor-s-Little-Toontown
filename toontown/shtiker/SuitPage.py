...
class SuitPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        icon = icons.find('**/money_icon')
        self.darkRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=(icon, icon, icon), image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.2, 10, -0.575), command=self.toggleRadar, extraArgs=[4])
        self.radarButtons.append(self.moneyRadarButton)
    ...
    def addQuotaLabel(self, panel):
        ...
        quotaLabel = DirectLabel(parent=panel, pos=(0.0, 0.0, -0.215), relief=None, state=DGG.DISABLED, text='%s of %s' % (count, quota), text_scale=0.045, text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getSuitFont())
    ...
    def setPanelStatus(self, panel, status):
        ...
        if status == COG_UNSEEN:
            ...
            suitFullName = 'Suit' # initial: SuitBattleGlobals.SuitAttributes[suitName]['name']
            ...
            if base.localAvatar.cogs[index] < COG_COMPLETE1:
                ...
                quota = str(COG_QUOTAS[1][index % SuitDNA.suitsPerDept])
            panel.quotaLabel['text'] = '%s of %s' % (count, quota)
