...
class SetMaxHP(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if not 15 <= maxhp <= 138:
            return "Can't set {}'s max laff to {}! Specify a value between 15 and 138.".format(toon.getName(), maxhp)
...
class SetEmblems(MagicWord):
    aliases = ["emblems", "setsprocketawards", "sprocketawards"]
...
class ToggleUnlimitedGags(MagicWord):
    aliases = ["setunlimitedgags", "unlimitedgags"]
        ...
        for do in simbase.air.doId2do.values():
            if isinstance(do, DistributedCashbotBossAI):
                if invoker.doId in do.involvedToons:
                    ...
                    break
        if not boss:
            return "You aren't battling the C.F.O.!"
...
class SkipCJ(MagicWord):
    desc = "Skips to the indicated round of the C.J."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    arguments = [("round", str, False, "next")]
        ...
        for do in simbase.air.doId2do.values():
            if isinstance(do, DistributedLawbotBossAI):
                if invoker.doId in do.involvedToons:
                    ...
                    break
        if not boss:
            return "You aren't battling the C.J.!"
...
class FillJury(MagicWord):
    desc = "Fills all of the chairs in the C.J.'s Jury Round."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
...
class SkipVP(MagicWord):
    desc = "Skips to the indicated round of the V.P."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    arguments = [("round", str, False, "next")]
        ...
        for do in simbase.air.doId2do.values():
            if isinstance(do, DistributedSellbotBossAI):
                if invoker.doId in do.involvedToons:
                    ...
                    break
        if not boss:
            return "You aren't battling the V.P.!"
...
class StunVP(MagicWord):
    desc = "Stuns the V.P. in the final round of his battle."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
        ...
        for do in simbase.air.doId2do.values():
            if isinstance(do, DistributedBossbotBossAI):
                if invoker.doId in do.involvedToons:
                    ...
                    break
        if not boss:
            return "You aren't battling the C.E.O.!"
...
class SpawnCogOldVersion(MagicWord):
    desc = "The previous version of the now SpawnCog magic word."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
...
class SpawnCog(MagicWord):
    aliases = ["cog", "spawnsuit", "suit"]
    desc = "Spawns a Cog with the defined level."
    example = "\"SpawnCog f 1\" will spawn a level one Flunky."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    arguments = [("suit", str, True), ("level", int, False, 1), ("skelecog", bool, False, False)]
...
class SpawnInvasion(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if not 1000 <= num <= 1000000:
            return "Can't the invasion amount to {}! Specify a value between 1,000 and 1,000,000.".format(num)
        ...
        if cmd == 'start':
            ...
            return "You didn't enter a valid command! Commands are ~SpawnInvasion start or stop."
...
class SetQuestProgress(MagicWord):
    aliases = ["qp", "questprogress", "setqp"]
...
class SetCogDisguise(MagicWord):
    aliases = ["cogdisguise", "cogsuit", "setcogsuit", "setsuit"]
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if corp not in corps:
            return "Invalid Cog department specified."
        ...
        types = SuitDNA.suitHeadTypes[9 * corpIndex:(9 * corpIndex) + 9]
        ...
        if typeIndex == 8:
            ...
            levelRange = range(9, 51)  # Last digit is exclusive.
        else:
            levelRange = range((typeIndex + 1), (typeIndex + 7))
        ...
        for levelBoost in [14, 19, 29, 39, 49]:
            if level <= levelBoost and not levelBoost > toon.getCogLevels()[corpIndex]:
                ...
                if toon.getMaxHp() >= 138:
                    continue
                toon.b_setMaxHp(toon.getMaxHp() + 1)
        # Lets be nice and give them a Toon-up or make them suffer.
...
class SetPouch(MagicWord):
    aliases = ["maxgagcarrylimit", "pouch", "setmaxgagcarrylimit"]
...
class SetNametagType(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        nametagTypeList = [(0, NametagGroup.CCNormal), (1, NametagGroup.CCNonPlayer), (2, NametagGroup.CCSuit)]
...
class SetSos(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        for npcId, npcName in {20000: 'Tutorial Tom',
                               999: 'Toon Tailor',
                               1000: 'Toon HQ',
 20001: 'Flippy',
 2001: 'Flippy',
...
class InstaDelivery(MagicWord):
    aliases = ["fastdel", "fastdelivery", "instadel"]
...
class SetTaskCarryLimit(MagicWord):
    aliases = ["taskcarrylimit", "settaskcarry", "setquestcarrylimit", "taskcarry", "setcarry"]
