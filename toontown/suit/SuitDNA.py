...
suitDepts = ['c',
 ...
 'm',
 's']
suitDeptFullnames = {'c': 'Bossbot',
 'l': 'Lawbot',
 'm': 'Cashbot',
 's': 'Sellbot',
 'g': 'Boardbot'}
suitDeptFullnamesP = {'c': 'Bossbots',
 'l': 'Lawbots',
 'm': 'Cashbots',
 's': 'Sellbots',
 'g': 'Boardbots'}
...
def getRandomSuitType(level, rng = random):
    return random.randint(max(level - 13, 1), min(level, 10))
    # I want to try this method out to see if, by chance, I can fix a problem concerning the new facility bosses' levels.
'''    try:
        returnval = random.randint(max(level - 13, 1), min(level, 10))
    except:
        returnval = 10
...
def getRandomSuitByDept(dept):
    ...
    return suitHeadTypes[suitsPerDept * deptNumber + random.randint(0, 10)]
